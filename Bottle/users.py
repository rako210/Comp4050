...
def check_login(db, user, password):
    ...
    p = password_hash(
        db, password, user)  # convert password to a hash in order to compare
    cursor = db.cursor()
...
def get_userID(db) -> int:
    """Get the id of the user currently signed in, if no user is signed in then
    'None' is returned."""
    return return_userID(db, session_user(db))
